...
/* verify.C */
bool verify (chordID key, dhash_ctype t, const char *buf, int len);
bool verify_content_hash (chordID key,  const char *buf, int len);
bool verify_keyhash (chordID key, const char *buf, int len);
...
ptr<dhash_block> get_block_contents (const char *data, 
				     unsigned int len, 
				     dhash_ctype t);
...
long keyhash_version (const char *value, unsigned int len);
