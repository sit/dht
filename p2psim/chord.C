...
Chord::stabilize()
{
  //get my predecessor's successor
  fix_successor ();

  IDMap succ1 = loctable->succ(me.id+1);
  assert(succ1.ip < 513);

  if (succ1.ip != oldsucc.ip) {

    //notify my succ of his predesessor change
    notify_args na;
    notify_ret nr;
    na.me = me;
...
LocTable::pred(Chord::CHID id) 
{
  ...
  while (1) {
    ...
    assert((rsz - deleted > 1) && (elm->id != me.id));
  }
